...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            const Text('语言囚徒', style: TextStyle(fontWeight: FontWeight.bold)),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _card(context,
              ...
              progress: 0.9,
              onTap: hasKey
                  ? () => _push(context, const DialogScreen())
                  : () => _needKey(context)),
          const SizedBox(height: 12),
          _card(context,
              icon: Icons.hearing,
          ...
          _card(context,
              icon: Icons.mic,
              title: '口语练习',
              sub: 'AI给出话题，开口说英语，AI评分',
              color: const Color(0xFFFF9800),
              progress: 0.7,
              onTap: hasKey
                  ? () => _push(context, const SpeakingScreen())
                  : () => _needKey(context)),
          const SizedBox(height: 12),
          _card(context,
              ...
              progress: 1.0,
              onTap: hasKey
                  ? () => _push(context, const TranslationScreen())
                  : () => _needKey(context)),
          const SizedBox(height: 20),
          const Text('工具',
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  void _needKey(BuildContext ctx) =>
  ...
  Widget _statsRow(AppState state) => Row(children: [
        _stat('今日练习', '${state.todayDialogs}次', Icons.today,
            const Color(0xFF4CAF50)),
        const SizedBox(width: 12),
        _stat('累计练习', '${state.totalDialogs}次', Icons.history,
            const Color(0xFF2196F3)),
        const SizedBox(width: 12),
        _stat('连续天数', '${state.streak}天', Icons.local_fire_department,
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      if (await ZhipuService.testKey(key)) {
        ...
        if (mounted) {
          _snack('✅ 验证成功，已保存！', false);
          Navigator.pop(context);
        }
      } else {
        if (mounted) _snack('❌ API Key无效', true);
      }
    } catch (e) {
      if (mounted) _snack('❌ 连接失败：$e', true);
    } finally {
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('智谱AI API Key',
                  style:
                      TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 6),
              const Text('访问 open.bigmodel.cn 注册获取免费API Key',
...
class _DialogScreenState extends State<DialogScreen>
  ...
  static const _scenes = [
    ...
    ('💼 面试', 'You are an English HR interviewer. Ask ONE interview question at a time, evaluate briefly, then ask the next. Correct mistakes in brackets [Correction: ...].'),
    ...
    ('🛍️ 购物', 'You are a shop owner in an English market. Shopping conversation one exchange at a time. Correct mistakes in brackets [Correction: ...].'),
  ];
  int _si = 0;
  ...
  Future<void> _initSpeech() async {
    if ((await Permission.microphone.request()).isGranted) {
      _speechOk = await _speech.initialize(
        ...
        onStatus: (s) {
          if (s == 'done' || s == 'notListening') _stopListen();
        },
      );
      setState(() {});
    }
    ...
    try {
      final r = await ZhipuService.chat(apiKey: key, messages: [
        {'role': 'system', 'content': _scenes[_si].$2},
        {
          'role': 'user',
          'content':
              'Start the conversation by greeting me and asking your first question.'
        },
      ]);
      setState(() {
        _msgs.add(_Msg(r, false));
        _waiting = true;
      });
      _scrollDown();
    } catch (e) {
  ...
  Future<void> _send([String? txt]) async {
    ...
    try {
      ...
      setState(() => _msgs.add(_Msg('⚠️ 失败: $e', false, err: true)));
    } finally {
      setState(() => _loading = false);
      _scrollDown();
    }
  ...
  Widget build(BuildContext context) => Scaffold(
          ...
          actions: [
            PopupMenuButton<int>(
                ...
                itemBuilder: (_) => _scenes
                    ...
                    .entries
                    .map((e) =>
                        PopupMenuItem(value: e.key, child: Text(e.value.$1)))
                    .toList())
          ],
        ),
        body: Column(children: [
          Container(
              ...
              child: Text(
                  _waiting ? '💬 轮到你回答！可打字或按麦克风说话' : '⏳ AI思考中...',
                  style: TextStyle(
                      fontSize: 12,
                      color: _waiting
          ...
          Container(
            ...
            child: Row(children: [
              ...
              AnimatedBuilder(
                  ...
                  builder: (_, __) => Transform.scale(
                        ...
                        child: GestureDetector(
                            onTap: !_loading ? _toggleListen : null,
                            child: Container(
                                width: 48,
                                height: 48,
                                decoration: BoxDecoration(
              ...
              const SizedBox(width: 8),
              GestureDetector(
                  onTap: (!_loading && !_listening) ? _send : null,
                  child: Container(
                      width: 48,
                      height: 48,
                      decoration: BoxDecoration(
                          color: (!_loading && !_listening)
                              ? const Color(0xFF2196F3)
  ...
  Widget _bubble(_Msg m) => Align(
        ...
        child: Container(
          margin: EdgeInsets.only(
              bottom: 10, left: m.isUser ? 48 : 0, right: m.isUser ? 0 : 48),
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(18).copyWith(
              bottomRight:
                  m.isUser ? const Radius.circular(4) : null,
              bottomLeft:
                  m.isUser ? null : const Radius.circular(4),
            ),
            boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 4)],
          ),
          child: Text(m.text,
...
class _ListeningScreenState extends State<ListeningScreen> {
  ...
  final _ctrl = TextEditingController();
  String _sentence = '', _chineseTip = '';
  bool _loading = false, _playing = false, _answered = false, _correct = false;
      ...
      final raw = await ZhipuService.chat(apiKey: key, messages: [
        {
          'role': 'system',
          'content':
              'Generate English listening dictation sentences. Return ONLY JSON: {"sentence":"...","chinese":"..."} No markdown.'
        },
        {
          'role': 'user',
          'content':
              '生成一个${_levels[_level]}难度的英语听写句子，10-20个单词，日常生活场景。JSON格式返回。'
        },
      ]);
      final data =
          jsonDecode(raw.replaceAll(RegExp(r'```json|```'), '').trim());
      setState(() {
  ...
  void _check() {
    ...
    final wa = input.split(' ').toSet();
    final wb = answer.split(' ').toSet();
    final sim = wb.isEmpty ? 0.0 : wa.intersection(wb).length / wb.length;
    setState(() {
      _answered = true;
      _correct = input == answer || sim > 0.85;
      if (_correct) _score++;
    });
  }
...
class _SpeakingScreenState extends State<SpeakingScreen>
  ...
  bool _speechOk = false, _listening = false, _loading = false;
  String _live = '', _topic = '', _topicCn = '', _spoken = '', _feedback = '';
  int _score = 0, _round = 0;
  late AnimationController _pulse;
  static const _levels = ['初级', '中级', '高级'];
  int _level = 0;
  ...
  Future<void> _nextTopic() async {
    ...
    try {
      final raw = await ZhipuService.chat(apiKey: key, messages: [
        {
          'role': 'system',
          'content':
              'Generate English speaking practice topics. Return ONLY JSON: {"topic":"...","chinese":"..."} No markdown.'
        },
        {
          'role': 'user',
          'content':
              '生成一个${_levels[_level]}难度的英语口语话题，要求用户用2-4句英语回答，话题贴近日常生活。JSON格式返回。'
        },
      ]);
      final data =
          jsonDecode(raw.replaceAll(RegExp(r'```json|```'), '').trim());
      setState(() {
        _topic = data['topic'] ?? '';
        _topicCn = data['chinese'] ?? '';
        _round++;
      });
    } catch (e) {
      setState(() => _topic = '加载失败，请重试');
    } finally {
      setState(() => _loading = false);
    }
  }
      ...
      final fb = await ZhipuService.chat(apiKey: key, messages: [
        {
          'role': 'system',
          'content':
              'You are an English speaking coach. Evaluate the spoken English. Give: 1) Score out of 10, 2) What was good, 3) Grammar/vocabulary corrections, 4) A better version. Reply in Chinese mixed with English examples.'
        },
        {
          'role': 'user',
          'content': '话题：$_topic\n\n用户的回答：$_spoken\n\n请评估并给出反馈。'
                  ...
                  Container(
                      ...
                      child: Column(children: [
                        AnimatedBuilder(
                            ...
                            builder: (_, __) => Transform.scale(
                                  scale: _listening
                                      ...
                                      : 1.0,
                                  child: GestureDetector(
                                      onTap: (!_loading && _feedback.isEmpty)
                                          ? _toggleListen
                                          : null,
                                      child: Container(
                                          width: 80,
                        ...
                        Text(
                            _listening
                                ? '正在录音，说完后停顿即可...'
                                : (_spoken.isNotEmpty ? '录音完成，点击评估' : '点击麦克风开始说话'),
                            style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
          padding: const EdgeInsets.all(20),
          child: Column(children: [
            _card('📅 今日练习次数', '${s.todayDialogs} 次',
                const Color(0xFF4CAF50)),
            const SizedBox(height: 16),
            _card('📊 累计练习次数', '${s.totalDialogs} 次',
                const Color(0xFF2196F3)),
            const SizedBox(height: 16),
